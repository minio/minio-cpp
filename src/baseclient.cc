...
minio::s3::BaseClient::ListMultipartUploads(ListMultipartUploadArgs args) {
  std::string region;
  if (GetRegionResponse resp = GetRegion(args.bucket, args.region)) {
    region = resp.region;
  } else {
  ...
  req.query_params.Add("uploads", "");
  req.query_params.Add("delimiter", args.delimiter);
  req.query_params.Add("max-uploads", args.max_uploads);
