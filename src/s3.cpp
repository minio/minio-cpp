...
S3Client::S3Client(const std::string &endpoint, const std::string &kid,
                   const std::string &sk)
    : endpoint(endpoint), keyID(kid), secret(sk), verbosity(0) {}
...
void S3Client::ParseBucketsList(list<Minio::S3::Bucket> &buckets,
...
void S3Client::ParseObjectsList(list<Minio::S3::Object> &objects,
                                const std::string &xml) {
  string::size_type crsr = 0;
  string data;
  ...
  while (ExtractXML(data, crsr, "Key", xml)) {
    ...
    if (ExtractXML(data, crsr, "LastModified", xml)) obj.lastModified = data;
    if (ExtractXML(data, crsr, "ETag", xml)) {
      // eTag starts and ends with &quot;, remove these
      // obj.eTag = data;
      obj.eTag = data.substr(6, data.size() - 12);
    }
    if (ExtractXML(data, crsr, "Size", xml)) obj.size = data;

    if (ExtractXML(data, crsr, "ID", xml)) obj.ownerID = data;
    if (ExtractXML(data, crsr, "DisplayName", xml)) obj.ownerDisplayName = data;

    if (ExtractXML(data, crsr, "StorageClass", xml)) obj.storageClass = data;
...
string S3Client::SignV2Request(const Minio::S3ClientIO &io,
...
struct WriteDataCB {
  ...
  size_t operator()(char *buf, size_t size, size_t nmemb) {
    return io.Write(buf, size, nmemb);
  }
};
...
struct HeaderCB {
  ...
  size_t operator()(char *buf, size_t size, size_t nmemb) {
    return io.HandleHeader(buf, size, nmemb);
  }
};
...
void S3Client::Submit(const std::string &url, const std::string &uri,
  ...
  if (verbosity >= 2) io.printProgress = true;

  try {
    // myCleanup required to do cleanup of used resources (RAII style) as
    // specified in curlpp docs.
    ...
    request.setOpt(new cURLpp::Options::WriteFunction(
        cURLpp::Types::WriteFunctionFunctor(WriteDataCB(io))));
    request.setOpt(new cURLpp::Options::HeaderFunction(
        cURLpp::Types::WriteFunctionFunctor(HeaderCB(io))));
    ...
    switch (method) {
      ...
      case Method::HTTP_PUT:
        if (io.bytesToPut > 0) {
          ...
          request.setOpt(new cURLpp::Options::ReadFunction(
              cURLpp::Types::ReadFunctionFunctor(ReadDataCB(io))));
        } else {
          request.setOpt(
              new cURLpp::Options::CustomRequest(methodToString(method)));
        }
        break;
...
void S3Client::PutObject(const std::string &bkt, const std::string &key,
  ...
  Submit(urlstrm.str(), bkt + PathSeparator + key, Method::HTTP_PUT, io,
         reqPtr);
}

void S3Client::PutObject(const std::string &bkt, const std::string &key,
                         const std::string &path, Minio::S3ClientIO &io,
                         S3Connection **reqPtr) {
...
Minio::S3::CompletePart S3Client::PutObject(const std::string &bkt,
                                            ...
                                            Minio::S3ClientIO &io,
                                            S3Connection **reqPtr) {
  if (partNumber <= 0) throw std::invalid_argument("received invalid value");
  std::ostringstream urlstrm;
  urlstrm << endpoint << PathSeparator << bkt << PathSeparator << key
  ...
  Submit(urlstrm.str(),
         bkt + PathSeparator + key + "?partNumber=" +
             std::to_string(partNumber) + "&uploadId=" + upload_id,
...
void S3Client::AbortMultipartUpload(const std::string &bkt,
  ...
  Submit(urlstrm.str(), bkt + PathSeparator + key + "?uploadId=" + upload_id,
         Method::HTTP_DELETE, io, reqPtr);
}

void S3Client::CompleteMultipartUpload(
    const std::string &bkt, const std::string &key,
    const std::string &upload_id,
    const std::list<Minio::S3::CompletePart> &parts, Minio::S3ClientIO &io,
    S3Connection **reqPtr) {
  std::ostringstream urlstrm;
  urlstrm << endpoint << PathSeparator << bkt << PathSeparator << key
          << "?uploadId=" + upload_id;
  ...
  for (auto &item : parts) {
    ...
    completeMultipartRequest.append(
        "<PartNumber>" + std::to_string(item.partNumber) + "</PartNumber>");
...
void S3Client::GetObject(const std::string &bkt, const std::string &key,
                         const int &partNumber, Minio::S3ClientIO &io,
                         S3Connection **reqPtr) {
  std::ostringstream urlstrm;
  urlstrm << endpoint << PathSeparator << bkt << PathSeparator << key
          << "?partNumber=" + std::to_string(partNumber);
  Submit(
      urlstrm.str(),
      bkt + PathSeparator + key + "?partNumber=" + std::to_string(partNumber),
      Method::HTTP_GET, io, reqPtr);
}
...
void S3Client::GetObject(const std::string &bkt, const std::string &key,
  ...
  std::ostringstream urlstrm;
  urlstrm << endpoint << PathSeparator << bkt << PathSeparator << key;
  Submit(urlstrm.str(), bkt + PathSeparator + key, Method::HTTP_GET, io,
         reqPtr);
}
...
void S3Client::StatObject(const std::string &bkt, const std::string &key,
  ...
  std::ostringstream urlstrm;
  urlstrm << endpoint << PathSeparator << bkt << PathSeparator << key;
  Submit(urlstrm.str(), bkt + PathSeparator + key, Method::HTTP_HEAD, io,
         reqPtr);
}
...
void S3Client::DeleteObject(const std::string &bkt, const std::string &key,
  ...
  std::ostringstream urlstrm;
  urlstrm << endpoint << PathSeparator << bkt << PathSeparator << key;
  Submit(urlstrm.str(), bkt + PathSeparator + key, Method::HTTP_DELETE, io,
         reqPtr);
}
...
void S3Client::CopyObject(const std::string &srcbkt, const std::string &srckey,
  ...
  io.reqHeaders.Update("x-amz-copy-source",
                       string(PathSeparator) + srcbkt + PathSeparator + srckey);
  ...
  Submit(urlstrm.str(), dstbkt + PathSeparator + dstkey, Method::HTTP_PUT, io,
         reqPtr);
}
...
void S3Client::MakeBucket(const std::string &bkt, Minio::S3ClientIO &io,
                          S3Connection **reqPtr) {
  std::ostringstream urlstrm;
...
void S3Client::ListObjects(const std::string &bkt, Minio::S3ClientIO &io,
                           S3Connection **reqPtr) {
  std::ostringstream urlstrm;
  urlstrm << endpoint << PathSeparator << bkt;
...
void S3Client::RemoveBucket(const std::string &bkt, Minio::S3ClientIO &io,
                            S3Connection **reqPtr) {
  std::ostringstream urlstrm;
  urlstrm << endpoint << PathSeparator << bkt;
