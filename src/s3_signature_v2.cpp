...
#define EVP_CREATE(c)            \
  EVP_MD_CTX _x##c, *c = &_x##c; \
  EVP_MD_CTX_init(c)
#define EVP_MD_CTX_destroy(c) EVP_MD_CTX_cleanup(c)
#define HMAC_CREATE(c)         \
  HMAC_CTX _x##c, *c = &_x##c; \
  HMAC_CTX_init(c)
...
size_t Minio::SignatureV2::ComputeMD5(uint8_t md5[EVP_MAX_MD_SIZE],
                                      std::istream &istrm) {
  EVP_CREATE(ctx);
...
std::string Minio::SignatureV2::GenerateSignature(
    const std::string &secret, const std::string &stringToSign) {
  HMAC_CREATE(ctx);
...
bool Minio::XML::ExtractXML(std::string &data, std::string::size_type &crsr,
...
bool Minio::XML::ExtractXMLXPath(std::string &data, const std::string &xpath,
  ...
  if (result) {
    ...
    for (pugi::xpath_node_set::const_iterator it = nodes.begin();
      ...
      for (pugi::xml_node child = node.node().first_child(); child;
