...
bool del(S3Client clnt, const std::string& aBucketName,
...
bool put(S3Client clnt, const std::string& aBucketName,
         const std::string& aFileName, const std::string& aKey) {
  S3ClientIO io;
...
bool get(S3Client clnt, const std::string& aBucketName, const std::string& aKey,
...
bool multipart(S3Client clnt, const std::string& aBucketName,
               const std::string& aKey, const std::string& aFileName,
  ...
  while (!fin.eof()) {
    ...
    while (!fin.eof() and left > 0) {
      ...
      if (!fin.eof() && fin.fail()) {
        clnt.AbortMultipartUpload(aBucketName, aKey, upload_id);
        cerr << "error reading " << rdsize << " bytes from " << aFileName
             << " left=" << left << endl;
        return false;
      }
    ...
    Minio::S3::CompletePart complPart =
        clnt.PutObject(aBucketName, aKey, partnum, upload_id, io);
    if (io.Failure()) {
      std::cerr << "ERROR: failed to put part " << partnum << " of "
                << aFileName << endl;
      std::cerr << "response:\n" << io << endl;
      std::cerr << "response body:\n" << io.response.str() << endl;
      clnt.AbortMultipartUpload(aBucketName, aKey, upload_id);
...
void usage() {
  ...
  std::cout << "  -m multipartsize: max size of each multipart upload"
            << std::endl;
}
...
int main(int argc, char** argv) {
  ...
  while ((c = getopt(argc, argv, "i:s:e:a:n:f:k:m:dh")) != -1) switch (c) {
      case 'i':
        lAccessKeyId = optarg;
        break;
      case 's':
  ...
  if (!lAccessKeyId) lAccessKeyId = getenv("ACCESS_KEY");

  if (!lSecretAccessKey) lSecretAccessKey = getenv("SECRET_KEY");

  if (!lEndpoint) lEndpoint = getenv("ENDPOINT");

  if (!lAccessKeyId) {
    std::cerr << "No Access Key given" << std::endl;
    std::cerr << "Either use -i as a command line argument or set ACCESS_KEY "
  ...
  if (!lSecretAccessKey) {
    ...
    std::cerr << "Either use -s as a command line argument or set SECRET_KEY "
                 "as an environmental variable"
              << std::endl;
    exit(1);
  }
  ...
  if (!lEndpoint) {
    ...
    std::cerr << "Either use -e as a command line argument or set ENDPOINT as "
